...
public final class InjectionPoint {
  ...
  public static InjectionPoint forConstructorOf(TypeLiteral<?> type) {
    ...
    if (filter.isWorthScanningForConstructors(Inject.class.getName(), rawType)) {
      for (Constructor<?> constructor : filter.getAllConstructors(Inject.class.getName(),
          rawType)) {
        boolean optional;
    ...
    try {
      ...
      if (Modifier.isPrivate(noArgConstructor.getModifiers()) && !Modifier.isPrivate(
   ...
   * @throws ConfigurationException if there is a malformed injection point on {@code type}, such
   * as
  ...
  static class InjectableField extends InjectableMember {
    ...
    @Override InjectionPoint toInjectionPoint() {
  ...
  static class OverrideIndex {
    ...
    void add(InjectableMethod injectableMethod) {
      ...
      if (bySignature != null) {
        ...
        Signature signature = injectableMethod.method == lastMethod ? lastSignature
   ...
   */
  private static void computeInjectableMembers(final TypeLiteral<?> type, boolean statics,
      Errors errors, InjectableMembers injectableMembers, OverrideIndex overrideIndex,
      HierarchyTraversalFilter filter) {
      ...
      for (Method method : allMethods) {
        if (isEligibleForInjection(method, statics)) {
          ...
          if (atInject != null) {
            ...
            if (checkForMisplacedBindingAnnotations(method, errors) || !isValidMethod(
                injectableMethod, errors)) {
