...
bash -e generate-latest-docs.sh
...
git clone --quiet --branch=gh-pages https://${GH_TOKEN}@github.com/markmarch/guice gh-pages > /dev/null
...
git add -f .
git commit -m "Latest javadoc & api-diffs on successful travis build $GITHUB_SHA auto-pushed to gh-pages"
